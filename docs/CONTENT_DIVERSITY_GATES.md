...
The gate rejects an empty published inventory, missing or non-content bodies, digest mismatches, duplicate IDs or links, any origin other than canonical `https://ownersoffice.co.jp`, non-public URL forms, invalid UTF-8 inventory data, and oversized input. HTML is parsed as an HTML5 fragment: inline markup preserves visible text adjacency, block elements create comparison boundaries, and non-content or intrinsically hidden subtrees (`script`, `style`, `template`, `noscript`, and `hidden`) are excluded. Accessibility/interaction attributes such as `aria-hidden` and `inert` do not hide visual text and therefore remain in the comparison. Because faithfully evaluating CSS cascade requires the page stylesheet, inline `display`, `visibility`, or `content-visibility` declarations (and ambiguous escaped/commented inline styles) reject the inventory fail-closed rather than guessing. Output contains only public provenance and the body digest, never the body. Keep the inventory in the runtime data location; do not commit published bodies to this repository.
